...
class IOWSModel(object):
    ...
    def make_plots(self, plt_tag):
        ...
        for (cc, i_ts) in enumerate(self.input_workload.LogData[0].timesignals):
            ...
            plt.bar(tot_signals[cc].xedge_bins[:-1], tot_signals[cc].yvalues_bins, tot_signals[cc].dx_bins, color=color_vec[:, cc])
        ...
        for (cc, i_ts) in enumerate(self.input_workload.LogData[0].timesignals):
            ...
            plt.bar(tot_signals[cc].xedge_bins[:-1], [i+1e-20 for i in err_vec],tot_signals[cc].dx_bins, color=color_vec[:, cc])
            plt.legend(['raw data', 'clustered'], loc=2)
            plt.ylabel(tot_signals[cc].name)
            plt.xlabel('time [s]')
    ...
    def create_scaled_workload(self, clustering_key, which_clust, scaling_factor):
        ...
        for iC in range(0, self._n_clusters):
            ...
            for tt in range(0, n_signals):
                ...
                ts_signal_x = np.ones(len(retrieved_yvals)) * (-999)  # just a dummy value (not actually needed..)
        ...
        for (ii, i_app) in enumerate(self.syntapp_list):
            ...
            # set start time to a random number (TODO: change this to more a sensible criterion)
        ...
        for (ii, i_sign) in enumerate(self.input_workload.total_metrics):
            tot_metrics_sums[ii] = sum(i_sign.yvalues_bins)
        ...
        for (ii, i_ts) in enumerate(self.aggregated_metrics):
            clust_metrics_sums[ii] = sum(i_ts.yvalues_bins)
    ...
    def export_scaled_workload(self):
        ...
        for i_app in self.syntapp_list:
            job_entry = i_app.make_kernels_from_ts(self.n_kernels)
