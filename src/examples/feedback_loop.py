...
def feedback_loop():
    ...
    scaling_factors_history_smooth = np.zeros(metrics_sums_history.shape)
    ...
    if not REPLAY_RUN:
        ...
        for i_count in range(0, n_iterations):
            ...
            scaling_factors_history_smooth = np.vstack([scaling_factors_history_smooth, sc_factors_vec])
        ...
        for i_count in range(0, n_iterations):
            ...
            # sc_factors_vec = relaxation_factor * (0.8*sc_factors_vec_raw + 0.2*scaling_factors_history_raw[-1, :])

            print "----------------------- iter = " + str(i_count) + " ---------------------------"
            print "metrics REF: ", reference_metrics_iter0
            print "metrics_sums: ", metrics_sums_history[-1, :]
            print "sc_factors_vec_raw: ", sc_factors_vec_raw
    ...
    np.savetxt(IOWS_DIR_BACKUP + '/' + RUN_TAG + 'scaling_factors_raw.txt', scaling_factors_history_raw, delimiter=",")
    np.savetxt(IOWS_DIR_BACKUP + '/' + RUN_TAG + 'scaling_factors_smooth.txt', scaling_factors_history_smooth, delimiter=",")
    # -------------------------------------------------------------------------------------
